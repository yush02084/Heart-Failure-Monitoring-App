...
| エージェント | 起動タイミング |
|---|---|
| **blueprint** | 複数セッション/複数担当にまたがる機能を計画するとき（統合・リファクタ・スプリント計画など） |
| **council** | 設計判断で悩んだとき（DBスキーマ変更、アーキテクチャ選択、トレードオフ）。4つの視点で構造的に議論する |
| **verification-loop** | 実装後に「本当に動いてるか」を体系的に検証するとき |
| **tdd-workflow** | テスト駆動でFlaskルート・アラートロジックを実装するとき |
| **search-first** | 実装前にパターン・事例・既存コードを調査するとき |
| **deep-research** | 心不全モニタリング・WCAG・医療UI設計について深掘りするとき |
| **security-scan** | PRマージ前のセキュリティ全体スキャン（security-reviewより網羅的） |
| **agentic-engineering** | エージェント的なタスク分解・ループ・並列処理を組む必要があるとき |
| **agent-sort** | 依頼内容を分析して適切なスキルに自動ルーティングしたいとき |

### 使い方
スキルは `skills/` に格納済み。Claudeが会話中にSKILL.mdを読んで自動起動する。
明示的に使いたい場合は「blueprintを使って計画して」「councilで判断して」と伝えるだけでOK。
