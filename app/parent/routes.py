...
def input():
    ...
    if form.validate_on_submit():
        ...
        try:
            ...
            # アラートレベルが注意・警戒のときwatcherにpush通知 + メール通知
            if alert_level >= 2:
                ...
                try:
                    ...
                    notify_watchers_push(parent_user_id=current_user.id, title=title, body=body)
                except Exception:
                    pass  # push失敗しても記録保存は成功扱い
                try:
                    from app.core.email_utils import notify_watchers_email
                    notify_watchers_email(parent_user_id=current_user.id, subject=title, body=body)
                except Exception:
                    pass  # email失敗しても記録保存は成功扱い
